...
public class Peer implements Comparable<Peer>, BandwidthListener {
    ...
    /**
     * ` Keeps state for in/out connections. Non-null when the handshake was successful, the
     * connection setup and runs. Do not access directly. All actions should be through Peer
     * methods.
     */
     ...
     * terminated.
     *
     * <p>When the connection is correctly started the connected() method of the given PeerListener
     * is called. If the connection ends or the connection could not be setup correctly the
    ...
    public void runConnection(
        ...
        try {
            ...
            if (_log.shouldDebug()) _log.debug("Start running the reader with " + toString());
    ...
    void disconnect() {
        ...
        if (s != null) {
            ...
            // This is blocking in streaming, so do this after closing the socket so it won't really
            // block
    ...
    public void downloaded(int size) {
        ...
        PeerState s = state;
        if (s != null) s.getBandwidthListener().downloaded(size);
    }
